...
os.environ[
    'PYSPARK_SUBMIT_ARGS'] = '--packages org.apache.spark_session:spark_session-sql-kafka-0-10_2.11:2.4.1 pyspark-shell'
...
def procss_df(events):
    ...
    mysql_cursor = mysql_conn.cursor()
    sql_update = update_statement.format(events["avg_SquareMeter"], events["avg_price_per_SM"], events["count_city"],
                                         events["city_code"])
    sql_insert = insert_statement.format(events["city_code"], events["avg_SquareMeter"], events["avg_price_per_SM"],
                                         events["count_city"])
    mysql_conn.commit()
    try:
...
def _init_kafka():
    ...
    bootstrap_server = 'Cnt7-naya-cdh63:9092'
    topics = "yad2"
